...
module.exports = function(args) {
    ...
    function cosStart(fileMap, cfgs, cdn) {
        ...
        .then(cosFileMap => {
            // console.log('getCosFiles: ')
            // console.log('fileMap: ', fileMap)
            // console.log('cosFileMap: ', cosFileMap)
        ...
        .then(allFiles => {
            // ＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊ debug
            // return cacheRefresh(cfgs, allFiles.uploadFiles)
            //     .then((res) => {
            //         if (res != false) {
            ...
            .then((filesMap) => {
                ...
                .then((res) => {
                    ...
                    QCSDK.request('RefreshCdnDir', dirs, (res) => {
                        res = JSON.parse(res);
                        // console.log(res)
                        if (res.codeDesc === 'Success') {
                            console.log('刷新cdn目录成功：', dirs)
                            // resolve(true);
                        } else {
                            console.log('刷新cdn目录失败：', dirs)
                            // reject(res);
                        }
                    })
                    // console.log(err);
...
function cacheRefresh(cfgs, filesMap) {
    ...
    console.log('cacheRefresh cfg:', cfgs)
    ...
    return new Promise((resolve, reject) => {
        ...
        filesMap.forEach( (file, filePath) => {
            ...
            ++i;
            ++ total
            //  避免 “(22043)单次提交刷新URL超出上限”
            if (i === pageSize) {
                const temp = JSON.parse(JSON.stringify(urls))
                const pageIdx = Math.round(total / pageSize)
                QCSDK.request('RefreshCdnUrl', temp, (res) => {
                    res = JSON.parse(res);
                    if (res.codeDesc === 'Success') {
                        console.log(pageIdx, '部分刷新成功：', res, temp)
                    } else {
                        console.warn(pageIdx, '部分刷新失败：', res, temp)
                    }
                })
                i = 0;
                urls = {};
        ...
        // const str = JSON.stringify(urls)
        // console.log('RefreshCdnUrl urls:', urls)
        // console.log('RefreshCdnUrl size:', str.length)
        QCSDK.request('RefreshCdnUrl', urls, (res) => {
            res = JSON.parse(res);
            if (res.codeDesc === 'Success') {
                console.log('共刷新cdn记录数：', total)
                resolve(true);
            } else {
                reject(res);
            }
            ...
            if (!localFileMap.has(key)) {
                ...
                await diffMd5(localFileMap.get(key)).then((md5) => {
                    if (md5 === eTag.substring(1, 33)) {
                        ...
                        console.log('diffMd5 same2: ', key, md5, eTag, eTag.substring(1, 33))
                    } else {
                        console.log('diffMd5 not same2: ', key, md5, eTag, eTag.substring(1, 33))
                    }
                })
            }
            ++i;
...
function checkConfigs(config) {
    let cfgs = config.deploy.filter(i => i.type === 'cos')[0] || config.deploy;
